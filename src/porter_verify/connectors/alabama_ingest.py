"""Stream Alabama business CSV or ZIP exports into the Alabama staging table."""
...
def _rows(path: Path) -> Iterator[dict]:
    if path.suffix.lower() == ".zip":
        with zipfile.ZipFile(path) as archive:
            ...
            if not names:
                raise ValueError("Alabama ZIP contains no CSV file")
            with (
...
def ingest_al_file(
    ...
    *,
    ...
    """Upsert every valid Alabama row; returns the number of entities written."""
    ...
    for row in _rows(Path(path)):
        ...
        if len(pending) >= batch_size:
            upsert_staging_rows(session, AlBusinessEntity, pending)
            session.commit()
            pending.clear()
            log.info("al_ingest_progress", rows=count)
    upsert_staging_rows(session, AlBusinessEntity, pending)
    session.commit()
    log.info("al_ingest_complete", rows=count)
    return count
