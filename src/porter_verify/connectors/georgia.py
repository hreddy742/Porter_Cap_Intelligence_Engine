"""Georgia Secretary of State business-entity parsing helpers."""
...
def parse_ga_date(value: str | None) -> date | None:
    if not value:
        return None
    text = value.strip()
...
def _source_url(entity_id: str) -> str | None:
    if not entity_id:
        return None
    return f"https://ecorp.sos.ga.gov/BusinessSearch/BusinessInformation?businessId={entity_id}"


def parse_ga_record(row: dict) -> GaBusinessRecord:
    entity_id = _first(row, "control number", "control_number", "entity id", "entity_id") or ""
    legal_name = _first(row, "business name", "entity name", "entity_name", "name") or ""

    return GaBusinessRecord(
        entity_id=entity_id,
        legal_name=legal_name,
        status_raw=_first(row, "status", "business status", "entity status"),
        entity_type=_first(row, "business type", "entity type", "type"),
        formation_date=parse_ga_date(_first(row, "date formed", "formation date", "created date")),
        ...
        jurisdiction=_first(row, "state of formation", "jurisdiction", "formation state"),
        source_record_url=_first(row, "source record url", "source_url") or _source_url(entity_id),
        officers=[],
        agent_name=_first(row, "registered agent name", "registered agent", "agent name"),
