"""Stream Mississippi business CSV or ZIP exports into the Mississippi staging table."""
...
def _rows(path: Path) -> Iterator[dict]:
    if path.suffix.lower() == ".zip":
        with zipfile.ZipFile(path) as archive:
            ...
            if not names:
                raise ValueError("Mississippi ZIP contains no CSV file")
            with (
...
def ingest_ms_file(
    ...
    *,
    ...
    """Upsert every valid Mississippi row; returns the number of entities written."""
    ...
    for row in _rows(Path(path)):
        ...
        if len(pending) >= batch_size:
            upsert_staging_rows(session, MsBusinessEntity, pending)
            session.commit()
            pending.clear()
            log.info("ms_ingest_progress", rows=count)
    upsert_staging_rows(session, MsBusinessEntity, pending)
    session.commit()
    log.info("ms_ingest_complete", rows=count)
    return count
