"""Stream Virginia business CSV or ZIP exports into the Virginia staging table."""
...
def _rows(path: Path) -> Iterator[dict]:
    if path.suffix.lower() == ".zip":
        with zipfile.ZipFile(path) as archive:
            ...
            if not names:
                raise ValueError("Virginia ZIP contains no CSV file")
            with (
...
def ingest_va_file(
    ...
    *,
    ...
    """Upsert every valid Virginia row; returns the number of entities written."""
    ...
    for row in _rows(Path(path)):
        ...
        if len(pending) >= batch_size:
            upsert_staging_rows(session, VaBusinessEntity, pending)
            session.commit()
            pending.clear()
            log.info("va_ingest_progress", rows=count)
    upsert_staging_rows(session, VaBusinessEntity, pending)
    session.commit()
    log.info("va_ingest_complete", rows=count)
    return count
