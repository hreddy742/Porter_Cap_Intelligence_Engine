"""Tennessee Secretary of State business-entity parsing helpers."""
...
def parse_tn_date(value: str | None) -> date | None:
    if not value:
        return None
    text = value.strip()
...
def _source_url(entity_id: str) -> str | None:
    if not entity_id:
        return None
    return f"https://tnbear.tn.gov/Ecommerce/FilingSearch.aspx?CN={entity_id}"


def parse_tn_record(row: dict) -> TnBusinessRecord:
