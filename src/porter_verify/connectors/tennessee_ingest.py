"""Stream Tennessee business CSV or ZIP exports into the Tennessee staging table."""
...
def _rows(path: Path) -> Iterator[dict]:
    if path.suffix.lower() == ".zip":
        with zipfile.ZipFile(path) as archive:
            ...
            if not names:
                raise ValueError("Tennessee ZIP contains no CSV file")
            with (
...
def ingest_tn_file(
    ...
    *,
    ...
    """Upsert every valid Tennessee row; returns the number of entities written."""
    ...
    for row in _rows(Path(path)):
        ...
        if len(pending) >= batch_size:
            upsert_staging_rows(session, TnBusinessEntity, pending)
            session.commit()
            pending.clear()
            log.info("tn_ingest_progress", rows=count)
    upsert_staging_rows(session, TnBusinessEntity, pending)
    session.commit()
    log.info("tn_ingest_complete", rows=count)
    return count
