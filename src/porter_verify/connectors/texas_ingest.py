"""Stream Texas business CSV or ZIP exports into the Texas staging table."""
...
def _rows(path: Path) -> Iterator[dict]:
    if path.suffix.lower() == ".zip":
        with zipfile.ZipFile(path) as archive:
            ...
            if not names:
                raise ValueError("Texas ZIP contains no CSV file")
            with (
...
def ingest_tx_file(
    ...
    *,
    ...
    """Upsert every valid Texas row; returns the number of entities written."""
    ...
    for row in _rows(Path(path)):
        ...
        if len(pending) >= batch_size:
            upsert_staging_rows(session, TxBusinessEntity, pending)
            session.commit()
            pending.clear()
            log.info("tx_ingest_progress", rows=count)
    upsert_staging_rows(session, TxBusinessEntity, pending)
    session.commit()
    log.info("tx_ingest_complete", rows=count)
    return count
