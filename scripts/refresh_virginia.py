"""Ingest an official Virginia SCC CSV or ZIP export."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh Virginia from an official file. No unverified bulk URL is embedded."
    )
    ...
    try:
        if args.url:
            ...
            path = Path(temporary.name) / f"virginia{suffix}"
        ...
        with factory() as session:
            count = ingest_va_file(session, path, limit=args.limit)
        print(f"Ingested {count:,} Virginia entities.")
    finally:
        if temporary:
            temporary.cleanup()
