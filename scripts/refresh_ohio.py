"""Ingest an official Ohio Secretary of State CSV or ZIP export."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh Ohio from an official file. No unverified download URL is embedded."
    )
    ...
    try:
        if args.url:
            ...
            path = Path(temporary.name) / f"ohio{suffix}"
        ...
        with factory() as session:
            count = ingest_oh_file(session, path, limit=args.limit)
        print(f"Ingested {count:,} Ohio entities.")
    finally:
        if temporary:
            temporary.cleanup()
