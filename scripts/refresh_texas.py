"""Ingest an official Texas Secretary of State CSV or ZIP export."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh Texas from an official file. No unverified bulk URL is embedded."
    )
    ...
    try:
        if args.url:
            ...
            path = Path(temporary.name) / f"texas{suffix}"
        ...
        with factory() as session:
            count = ingest_tx_file(session, path, limit=args.limit)
        print(f"Ingested {count:,} Texas entities.")
    finally:
        if temporary:
            temporary.cleanup()
