"""Ingest an official Georgia Secretary of State CSV or ZIP export."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh Georgia from an official file. No unverified bulk URL is embedded."
    )
    ...
    try:
        if args.url:
            ...
            path = Path(temporary.name) / f"georgia{suffix}"
        ...
        with factory() as session:
            count = ingest_ga_file(session, path, limit=args.limit)
        print(f"Ingested {count:,} Georgia entities.")
    finally:
        if temporary:
            temporary.cleanup()
