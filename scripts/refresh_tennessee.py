"""Ingest an official Tennessee Secretary of State CSV or ZIP export."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh Tennessee from an official file. No unverified bulk URL is embedded."
    )
    ...
    try:
        if args.url:
            ...
            path = Path(temporary.name) / f"tennessee{suffix}"
        ...
        with factory() as session:
            count = ingest_tn_file(session, path, limit=args.limit)
        print(f"Ingested {count:,} Tennessee entities.")
    finally:
        if temporary:
            temporary.cleanup()
