"""Ingest an official Alabama Secretary of State CSV or ZIP export."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh Alabama from an official file. No unverified bulk URL is embedded."
    )
    ...
    try:
        if args.url:
            ...
            path = Path(temporary.name) / f"alabama{suffix}"
        ...
        with factory() as session:
            count = ingest_al_file(session, path, limit=args.limit)
        print(f"Ingested {count:,} Alabama entities.")
    finally:
        if temporary:
            temporary.cleanup()
